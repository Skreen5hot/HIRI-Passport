...
**Status:** DRAFT — NOT EFFECTIVE; no selection or approval may be inferred

**Prepared:** 2026-07-20

**Proposed approver:** Aaron A Damiano, Project Owner

**Parent authority:** RHP-DR-001 signed tag `RHP-DR-001-approved-2026-07-20`

## 0. Purpose

RHP-DR-001 authorizes the release identity and risk boundary. This record asks only for the remaining technical-policy choices that materially affect the production architecture. It does not authorize real-data deployment, collect participant data, close independent review, or constitute the final go/no-go.

Every unselected decision retains the safe state defined in `docs/rhp/release-contract.md`:
...
- [ ] **A. Project-preview resource set after independent technical review (recommended).** The project owner may approve one exact versioned manifest after a reviewer other than the byte author verifies specification correspondence, hashes, schemas, vectors, and placeholder exclusion. Resources remain project-preview resources and do not imply Candidate or normative standards status.
...
**Recommendation rationale:** A permits a reproducible preview without misrepresenting project bytes as ecosystem-normative. B is safest but prevents a functional real-key preview for an indeterminate period. C is appropriate only if such a body actually exists.

**If A is selected, required conditions:**
...
- [ ] **A. Empty trust sets with warned identity-unknown requests (recommended).** Approve empty issuer, BVS, identity-anchor, resolver, current-head, remote-resource, HTTPS-delivery, credential-schema, and relying-party-policy sets. Disclosure Requests enter only through explicit local file selection or paste; the preview exposes no live verifier request endpoint. A request whose cryptography and lifecycle evidence validate may reach consent with a prominent `verifier identity unknown` warning. Output is limited to explicit file or clipboard delivery; policy remains `not-evaluated`.
...
**Recommendation rationale:** A exercises the holder consent/signing boundary while preserving honest evidence separation and avoiding invented organizational trust or server endpoints. It exposes holders to untrusted request purposes, so validation, inert rendering, complete-public preview, decline, and local authentication are mandatory.

**If A is selected, required UI facts:**
...
- [ ] **A. Evidence-first mobile/desktop Chromium pilot (recommended).** Build against capability contracts, then physically test one Windows desktop Chromium pair and one Android Chromium pair. Approve only the exact tested version ranges that pass the complete matrix. Every other platform remains inspect-only until separately tested and approved.
...
**Recommendation rationale:** A preserves the mobile-first product intent without pretending viewport emulation is device evidence. It limits initial assurance to two evidence packages and avoids assuming that browser engines share key/storage behavior.

**Conditions applicable to every option:**
...
- [ ] **A. Non-extractable browser keys plus mandatory WebAuthn user verification (recommended).** Real authority creation is available only where Ed25519/X25519 private keys are generated non-extractably and durably stored. A fresh WebAuthn user-verification ceremony gates authority creation, presentation signing, rotation, device changes, approved backup/restore, and destructive key deletion. Cancellation/failure has no sensitive side effect. There is no weaker fallback.
...
**Recommendation rationale:** A adds a fresh local user-presence/verification gate without mislabeling WebAuthn as Passport evidence or hardware protection. It narrows supported platforms and does not protect against all same-origin script or device compromise.

**Conditions applicable to A or B:**
...
**Recommendation rationale:** A is aligned with RHP-DR-001 D5-A's non-durable, disposable authority and minimizes the highest-risk key-export/recovery code in the first preview. It produces the clearest loss consequence and the smallest mobile-first implementation.
...
## 7. Combined recommendation

The preparer's recommended combination is:
...
Complete only after selecting exactly one option in every section:
...
| Review/expiry date | not supplied |
| Selected option summary | not supplied |
| Parent decision | RHP-DR-001 |
| Exact record commit | generated after completion |
| Exact file SHA-256 | generated after completion |
| Signature mechanism | GPG-signed Git tag recommended |
| Signer fingerprint | `07171B3AF6042998D1ADDEE0DE640D2A3317B186` |
| Threat-model re-review required before deployment | Yes — signing this policy triggers re-review under `docs/rhp/threat-and-privacy-model.md` |

This draft becomes effective only after mechanical completeness checks, an exact committed-byte SHA-256, and external signature verification. Typed selections or initials without that binding are not approval.
