...
for (const requiredStatement of ["Platform availability — limited and evidence-based", "Public access does not mean every browser or device is supported", "Selective disclosure — not supported", "Confirmed-current status — not available in this preview", "Recovery after total key loss — not supported", "Proximity transport — not supported", "Blocking design issues remain open"]) if (!preview.includes(requiredStatement)) throw new Error(`Preview information page is missing approved statement: ${requiredStatement}`);
